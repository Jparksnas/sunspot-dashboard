...
def plot_advanced_sunspot_visualizations(df, sunactivity_col='SUNACTIVITY'):
    ...
    # (a) 전체 시계열 라인 차트
    axs[0, 0].plot(df.index, df[sunactivity_col], color='royalblue', linewidth=1)
    ...
    if len(data) > 0:  # 데이터가 있는지 확인
        ...
        axs[0, 1].hist(data, bins=20, density=True, alpha=0.5, color='gray', edgecolor='black', label='Histogram')
    ...
    try:
        ...
        if not df_20th.empty:
            axs[1, 0].boxplot(df_20th[sunactivity_col].dropna(), vert=True) 
    except:
